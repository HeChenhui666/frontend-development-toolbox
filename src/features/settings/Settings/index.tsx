...
const Settings: React.FC<SettingsProps> = memo(({ onClose, embedded = false, chatRelayPanelInTab = false, onlyTabs }) => {
  ...
  // 选择性导入的类别名称
  const CATEGORY_LABELS: Record<CacheType, string> = {
    theme: '主题设置',
    presets: 'URL预设参数',
    games: '游戏积分',
    preferences: '用户偏好',
  ...
  // 执行选择性导入并刷新 UI
  const executeImport = useCallback((jsonString: string, selectedCategories?: CacheType[]) => {
    const result = importUserConfig(jsonString, selectedCategories);
    if (result.success) {
  ...
  // 弹出选择性导入对话框
  const showSelectiveImportModal = useCallback((jsonString: string) => {
    const parsed = parseImportCategories(jsonString);
    if (!parsed.success || !parsed.categories) {
      antdMessage.error(parsed.message);
      return;
    }
    const availableCategories = Object.keys(parsed.categories) as CacheType[];
    const selectedSet = new Set<CacheType>(availableCategories);
        ...
        <div style={{ marginTop: 8 }}>
          <Checkbox.Group
            ...
            onChange={(values: string[]) => {
              selectedSet.clear();
              (values as CacheType[]).forEach((v) => selectedSet.add(v));
            }}
            style={{ display: 'flex', flexDirection: 'column', gap: 8 }}
          >
            {availableCategories.map((cat) => (
              <Checkbox key={cat} value={cat}>
                {CATEGORY_LABELS[cat]}（{parsed.categories![cat]} 项）
              </Checkbox>
            ))}
          </Checkbox.Group>
          <Text type="secondary" style={{ display: 'block', marginTop: 12, fontSize: 12 }}>
            勾选的类别将覆盖本地对应设置，未勾选的保持不变。
          </Text>
        </div>
      ),
      okText: '导入选中项',
      cancelText: '取消',
      onOk: () => executeImport(jsonString, Array.from(selectedSet)),
    });
  }, [executeImport]);
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (event) => {
        try {
          ...
          showSelectiveImportModal(jsonString);
        } catch (error) {
          antdMessage.error('读取文件失败');
          console.error('Import failed:', error);
        }
      };
      reader.onerror = () => antdMessage.error('读取文件失败');
      reader.readAsText(file);
    };
    input.click();
  }, [showSelectiveImportModal]);
  ...
  const handleImportFromClipboard = useCallback(async () => {
    ...
    try {
      if (navigator.clipboard?.readText) {
        ...
        if (clipboardText?.trim()) {
          JSON.parse(clipboardText); // 验证 JSON
          showSelectiveImportModal(clipboardText.trim());
          return;
        }
      }
    } catch {
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          ...
          <TextArea
            ...
            onPaste={() => {
              setTimeout(() => {
                const el = document.getElementById('config-input-textarea') as HTMLTextAreaElement;
                if (el) inputValue = el.value;
              }, 0);
            }}
          />
        </div>
      ...
      onOk: () => {
        ...
        showSelectiveImportModal(inputValue.trim());
      },
    });
  }, [showSelectiveImportModal]);
