...
| 项目        | 说明                                                                                                                   |
| --------- | -------------------------------------------------------------------------------------------------------------------- |
| **当前版本**  | **1.8.6**（与仓库根目录 `package.json`、`manifest.json` 中 `version` 一致；以 **`npm run build` 后 `dist/manifest.json`** 为准。）     |
...
* Unicode 编码（转义 / 还原）、UTF-8 编解码、URL 编码（encodeURIComponent / decodeURIComponent）等常用文本变换
* 实时转换，支持复制输出
